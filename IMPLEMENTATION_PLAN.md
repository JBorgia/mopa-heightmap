...
- New module `mopa/color_picker.py`:
...
- `mopa/pipeline.py` — `Stage`, `Pipeline`, `PipelineRunner`, hash-based cache.
- `mopa/preview.py` — downscale-on-write helpers, ROI preview support (preview a 256² crop at full DPI for material-test workflows).
...
- `mopa/autofit.py`: change suggestion `detail_mode="both", detail_strength=0.4` → `detail_mode="highpass", detail_strength=0.10`.
...
- New module `mopa/backbones/depth_anything.py` exposing `DepthAnythingV2Backbone` with `.predict(rgb_uint8) -> float32 depth (H,W)`.
...
- `mopa/subject_mask.py`: `SubjectMasker(backend="birefnet"|"rembg")` returning float32 alpha in [0,1].
...
- `mopa/normals.py`: `NormalEstimator(backend="dsine"|"marigold")` → `(N,3)` unit vectors per pixel.
- `mopa/frankot_chellappa.py`: pure NumPy FFT integration of `(p, q) = (-Nx/Nz, -Ny/Nz)` → height. ~50 ms for 1024². Symmetric padding to suppress edge ringing. Self-contained, fully unit-testable on synthetic surfaces (sphere, ramp, sinusoid).
...
- `mopa/regions.py`: `RegionSegmenter` returning a label map (face / hair / clothing / skin / accessory / bg).
...
- **5c — Pass planner** in `mopa/stages.py`:
...
> Replaces the Gradio-based [`ui/mopa_studio.py`](ui/mopa_studio.py) and [`ui/mopa_wizard.py`](ui/mopa_wizard.py) front-ends with a real desktop-class single-page app. The Python pipeline (everything under `mopa/`) becomes a headless service exposed over HTTP + WebSocket; the Gradio UIs stay shipped during the transition as the fallback surface.
...
| Backend | **FastAPI** wrapping the existing `mopa.service` |
...
- Reuse `mopa.service.InferenceService` verbatim — the FastAPI handlers are thin wrappers over the same `_service()` singleton the Gradio UIs already call.
